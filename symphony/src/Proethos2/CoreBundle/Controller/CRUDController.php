...
class CRUDController extends Controller
{
    ...
    public function listCommitteeProtocolAction()
    {
        ...
        if( $collection_object ) {
            $query = $protocol_repository->createQueryBuilder('p')
               ...
               ->where("(s.title LIKE :query OR s.descriptors LIKE :query OR s.keywords LIKE :query) AND p.status IN (:status) AND c.id = :collection")
    ...
    public function listInvestigatorProtocolAction()
    {
        ...
        $query = $protocol_repository->createQueryBuilder('p')
           ...
           ->where("(s.title LIKE :query OR s.descriptors LIKE :query OR s.keywords LIKE :query) AND p.status IN (:status) AND ((s.owner = :owner OR t = :owner) OR r.member = :owner)")
           ->orderBy("p.created", 'DESC')
           ->setParameter('query', "%". $search_query ."%")
           ->setParameter('status', $status_array)
...
*/
    ...
    public function listCommitteeUserAction()
    {
        ...
        if($this->getRequest()->isMethod('POST')) {
            ...
            ->setSubject("[EXP] " . $translator->trans("Set your password"))
    ...
    public function updateCommitteeUserAction($user_id)
    {
        ...
        if($this->getRequest()->isMethod('POST')) {
            ...
            if(isset($post_data['status']) && !$user_status) {
                ...
                $message = \Swift_Message::newInstance()
                ->setSubject("[EXP] " . $translator->trans("Confirmation of valid access to the Experience Reports platform"))
                ->setFrom($util->getConfiguration('committee.email'))
                ->setTo($user->getEmail())
                ->setBody(
                    $body
    ...
    public function listContactAction()
    {
        ...
        if($this->getRequest()->isMethod('POST')) {
            ...
            $message = \Swift_Message::newInstance()
            ->setSubject("[EXP] " . $translator->trans("Message from plataform."))
    ...
    /**
     * @Route("/admin/controlled-list/tags", name="crud_admin_controlled_list_tags_list")
     * @Template()
     */
    public function listControlledListTagsAction()
    {
        $output = array();
        $request = $this->getRequest();
        $session = $request->getSession();
        ...
        $item_repository = $em->getRepository('Proethos2ModelBundle:Tags');
        $trans_repository = $em->getRepository('Gedmo\\Translatable\\Entity\\Translation');
        ...
        if($this->getRequest()->isMethod('POST')) {
            ...
            $session->getFlashBag()->add('success', $translator->trans("Item created with success."));
            return $this->redirectToRoute('crud_admin_controlled_list_tags_list', array(), 301);
        }
    ...
    /**
     * @Route("/admin/controlled-list/tags/{item_id}", name="crud_admin_controlled_list_tags_update")
     * @Template()
     */
    public function updateControlledListTagsAction($item_id)
    {
        $output = array();
        $request = $this->getRequest();
        $session = $request->getSession();
